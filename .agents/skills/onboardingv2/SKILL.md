...
If that fails, check `~/.agents/skills/` and `~/.cursor/skills/` for a cached copy, or follow `mcp-configure` inline using [MCP Config Templates](references/mcp-config-templates.md). After it succeeds, call `get-project` once (`projectKey: "default"`) and store `projectKey` and `envKey` (`test`).
