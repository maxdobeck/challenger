...
**Setup:** [Hosted MCP](https://launchdarkly.com/docs/home/getting-started/mcp-hosted) — uses OAuth; no tokens or API keys needed in config files. See the [MCP server docs](https://github.com/launchdarkly/mcp-server) for editor-specific setup instructions.

## Step 2: Fill in the Template

Replace all `{PLACEHOLDER}` values with the actual values from the onboarding session (gather any you did not write down earlier from **Project settings → Environments** in the LaunchDarkly dashboard or from MCP tools):
...
- `{PROJECT_KEY}`: The LaunchDarkly **project** key (used in dashboard URLs and MCP tool calls)
- `{ENV_KEY}`: The LaunchDarkly **environment** key for the environment whose SDK key / client-side ID you used (e.g., `production`, `test`, `development`) — used in dashboard URLs scoped per environment
